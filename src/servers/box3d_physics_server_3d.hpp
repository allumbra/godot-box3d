...
// _step()/_flush_queries() across every active Box3DSpace3D.
...
public:
	...
	Box3DAreaImpl3D* get_area(const RID& p_rid) const { return area_owner.get_or_null(p_rid); }
	...
	void _soft_body_remove_collision_exception(const RID& p_body, const RID& p_excepted_body) override;
	...
	void _soft_body_set_linear_stiffness(const RID& p_body, double p_stiffness) override;
	double _soft_body_get_linear_stiffness(const RID& p_body) const override;
	void _soft_body_set_pressure_coefficient(const RID& p_body, double p_pressure_coefficient) override;
	double _soft_body_get_pressure_coefficient(const RID& p_body) const override;
	void _soft_body_set_damping_coefficient(const RID& p_body, double p_damping_coefficient) override;
